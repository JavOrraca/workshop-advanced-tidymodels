...
# Review columns from above transformation
colnames(bake(prep(hotel_rec_javi), new_data = hotel_train))
```
...
collect_metrics(hotel_lm_res_javi)
...
lm_cv_pred <- collect_predictions(hotel_lm_res_javi)
...
cal_plot_regression(hotel_lm_res_javi, alpha = 1 / 5)
```
